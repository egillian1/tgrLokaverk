...
window.onload = function init() {
  ...
  let ang = vec2(270.0, 0.0);
  let pos = vec3(0.0, 0.0, 0.0);
  let dir = vec3(0.0, 0.0, 1.0);
  player = new Ship(ang, pos, dir, pos, spaceshipPoints);
  ...
  window.addEventListener("keydown", function(e){
    ...
    if(key == 17){
      ...
      player.position[0] += 0.1 * player.direction[0];
      player.position[2] += 0.1 * player.direction[2];
    } else if(key == 32){
      ...
      player.angles[0] += degreesPerMovement;
      player.direction[0] = Math.cos(radians(player.angles[0]));
      player.direction[2] = Math.sin(radians(player.angles[0]));
    } else if (key == 68) {
...
class Ship {
  constructor(angles, position, direction, velocity, pointMatrix){
    this.angles = angles;
...
function render(){
  ...
  */
  ...
  let tmp = vec3(player.position[0] + player.direction[0], 0.0, player.position[2] + player.direction[2]);
  ctm = lookAt(player.position, tmp, vec3(0.0, 1.0, 0.0));
  //ctm = rpyView(player.direction[2], player.direction[1], player.direction[0], player.position)
